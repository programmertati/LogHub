                                ...
                                <li class="kartu-loghub" data-id="{{ $dataKartu->id }}"
                                    ...
                                    <div class="dropdown dropdown-action aksi-card" id="aksi-card{{ $dataKartu->id }}"
                                        ...
                                        <div class="dropdown-menu dropdown-menu-right">
                                            ...
                                            <a href="#" class="dropdown-item"
                                                onclick="deleteCardModal2('{{ $dataKartu->id }}', '{{ $dataKartu->name }}', '{{ $dataKolom->name }}', '{{ route('hapusKartu', ['card_id' => $dataKartu->id]) }}');">
                                                <i class='fa fa-trash-o m-r-5'></i> Delete
                                            </a>
                                            <a href="#" class="dropdown-item"
                                                ...
                                                @else
                                                    <div id="iconChecklist-{{ $dataKartu->id }}"
                                                        class="progress-checklist-light hidden
                                                            @foreach ($result_tema as $mode_tema)
                                                                @if ($mode_tema->tema_aplikasi == 'Gelap')
                                                                    progress-checklist-dark hidden
                                                                @endif @endforeach
                                                            @php
$totalPercentage = 0;
                                                                $totalCount = 0;
        ...
        document.addEventListener('DOMContentLoaded', function() {
            document.querySelectorAll('form[id^="addCardForm"]').forEach(form => {
                form.addEventListener('submit', function(event) {
                    addCardScript(event, this.dataset.columnId);
                });
            });
        });
    </script> --}}
    ...
    <div id="updateBoard" class="modal custom-modal fade" role="dialog">
        ...
        <div id="addCol" class="modal custom-modal fade" role="dialog">
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form id="addColForm"
                            action="{{ route('addCol', ['board_id' => $board->id, 'team_id' => $board->team_id]) }}"
        ...
        @include('admin.script-modal-card')
        <!-- /Isian Kartu Modal -->
        ...
        @include('allrole.memuat-modal-pulihkan-kolom')
        <!-- /Pulihkan Kolom Modal --> --}}
        ...
        <style>
            ...
            @foreach ($result_tema as $sql_mode => $mode_tema)
                @if ($mode_tema->tema_aplikasi == 'Gelap')
                    ...
                    p {
                        color: {{ $mode_tema->warna_mode }} !important
                    }

                    .custom-modal .tag-list {
                        color: {{ $mode_tema->warna_sistem_tulisan }} !important
                    }
                    ...
                    .border-dark {
                        border-color: {{ $mode_tema->warna_sistem_tulisan }} !important;
                    }
                    ...
                    .border-dark:hover {
                        background-color: {{ $mode_tema->warna_sistem }} !important;
                        cursor: pointer;
                    }
                    ...
                    input[type=text] {
                        color: white;
                        background-color: {{ $mode_tema->warna_mode }} !important
                    }
