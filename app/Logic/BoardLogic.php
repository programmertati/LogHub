...
class BoardLogic
{
    ...
    {
        $columns = collect();
        $board = Board::with('columns.cards')->find($board_id);

        $board->columns->each(function ($row) {
            $row->setHidden(['nextCard', "created_at", "updated_at", "column_id", "previous_id", "next_id"]);
            $row->cards->each(function ($row) {
                $row->setHidden(['nextColumn', 'updated_at', 'created_at', 'previous_id', "next_id", "board_id"]);
            });
        });
