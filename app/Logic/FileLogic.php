...
class FileLogic
{
    ...
    // Tempat Penyimpanan Gambar Pengguna //
    public function storeUserImage(int $user_id, Request $request, string $field_name)
    {
        ...
        if ($user->avatar != null) {
            $old_image_path = str_replace($this->prefix, "", $user->avatar);
            Storage::disk($this->public)->delete($old_image_path);
        }
        $user->avatar = $this->prefix . $new_image_path;
        $user->save();
    }
    // /Tempat Penyimpanan Gambar Pengguna //
}